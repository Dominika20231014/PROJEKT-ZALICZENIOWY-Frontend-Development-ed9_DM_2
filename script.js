...
function searchCarsByBrand() {
  const selectedOptions = Array.from(
    document.getElementById("brandSelect").selectedOptions
  );
...
document
  ...
  .addEventListener("click", searchCarsByBrand);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const backToCarListButton = document.getElementById("backToCarListButton");
  ...
  function init() {
    ...
    // Sprawdzenie,czy wyświetlany jest formularz zakupu
    const isPurchaseFormDisplayed =
      document.getElementById("purchaseForm").style.display === "block";

    // Ustawionej flagi na true
    if (
      isPurchaseFormDisplayed &&
      !sessionStorage.getItem("enteredPurchaseForm")
    ) {
  ...
  function validateDeliveryDate() {
    ...
    if (selectedDate.getDay() === 0) {
      errorMessageContainer.textContent =
        "Proszę wybrać datę inną niż niedziela.";
  ...
  function generateAccessoriesList() {
    ...
    accessories.forEach((accessory) => {
      ...
      removeButton.addEventListener("click", (event) => {
        ...
        const existingAccessoryIndex = selectedAccessories.findIndex(
          (acc) => acc.id === accessory.id
        );
  ...
  function updateOrderSummary(car, accessories) {
    ...
    if (accessories.length > 0) {
      ...
      const totalAccessoriesPrice = accessories.reduce(
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (
      ...
      const totalAccessoriesPrice = selectedAccessories.reduce(
        (total, acc) => total + acc.price * acc.quantity,
        0
      );
      const totalPriceWithAccessories =
        selectedCar.price + totalAccessoriesPrice;
      document.getElementById(
        "totalPrice"
      ).textContent = `Cena całkowita: $${totalPriceWithAccessories}`;
      form.reset();
      setTimeout(() => {
        window.scrollTo(0, 0);
      ...
      document.getElementById("errorMessage").textContent =
        "Błąd Proszę uzupełnić pola wymagane";
  ...
  // Funkcja do obsługi przycisku "Powrót do sklepu" w formularzu podziękowań
  backToCarListButton.addEventListener("click", () => {
    confirmation.style.display = "none";
    carList.style.display = "flex";
    carList.style.flexWrap = "wrap";
    header.style.display = "block"; // Przywrócenie nagłówka
  ...
  document
    ...
    .addEventListener("submit", (event) => {
      ...
      if (selectedDate.getDay() === 0) {
        errorMessageContainer.textContent =
          "Niestety, dostawa w niedzielę nie jest możliwa. Proszę wybrać inną datę.";
  ...
  document
    ...
    .addEventListener("change", validateDeliveryDate);
});
